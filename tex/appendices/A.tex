...
sa deuxième version(en ~\ref{sec:semantic-web-owl-owl2}).
...
\subsubsection{Les exigences pour la définition des langages
  d'ontologies}
\label{sec:sec:semantic-web-owl-ontolgies-requirements}

\subsubsection{Les limitations d'expressivité du langageRDFS}
\label{sec:sec:semantic-web-owl-ontolgies-rdfs-limitaions}

\subsection{Le langage OWL2}
\label{sec:semantic-web-owl-owl2}
\acrshort{owl2}~\cite{w3c2009owl} est une révision du langage
\acrshort{owl}~\cite{mcguinness2004owl} de définition des ontolgies,
il ajoute de nouvelles fonctionnalités par rapport à \acrshort{owl},
incluant:

\SpecialItem
\begin{itemize}
\item Des types de donnés plus riches.
\item La définition dee restrictions de cardinalité qualifiés.
\item Les propriétés asymétriques, réflexives et disjoints.
\end{itemize}
\enddescription
