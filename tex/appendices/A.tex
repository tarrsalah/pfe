...
illustre graphiquement les relations qui existent entre les différents
composants de base du langage \acrshort{rdfs}.

% * rdfs vs rdf layers and why rdfs is built upon rdf?
\input{figs/A/rdfs-vs-rdf-layers.tex}

% Une ontologie \acrshort{rdfs} s'écrit sous forme de triplets
% \acrshort{rdf} qui vont ainsi définir des identfiants pour ses
% différentes classes et propriétés, ceux-ci étant uniques puisque basés
% sur des \acrshort{uri}'s. représente une ontologie modélisant une
% partie des classes et propriétés que nous avons présentées dans
% l'exemple précédent. Il introduit également la possibilité dans une
% ontologie d’utiliser et d’étendre des classes et propriétés définies
% dans d'autres modèles.

% * rdfs examples (foaf, schema.org)

% * rdfs inference

% * why rdfs is not enough, and introducing owl
