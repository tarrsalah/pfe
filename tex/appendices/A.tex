...
classes, des propriétés, des instances de classes (individus) et les
relations entre ces instances:

\renewcommand{\descriptionlabel}[1]{\hspace{0.5cm}\textbullet~\textsf{#1}}
\begin{description}
\item[Les classes]: Les classes fournissent un mécanisme d'abstraction
  permettant de regrouper des ressources avec des caractéristiques
  similaires. Par example:\medskip
...
\end{lstlisting}
  ...
  On peut également exprimer des relations plus complexes entres les
  classes, commes l'intersection \texttt{owl:intersectionOf} , l'union
  (\texttt{owl:unionOf}) ou l'équivalence
  (\texttt{owl:equivalentClass}) entre de deux ou plusieurs classes.

\newpage

\item[Les propriétés]: Le langage \acrshort{owl2} distingue deux
  catégories de propriétés définissables par un concepteur
  d'ontologies: Les propriétés d'objets
  (\texttt{owl:ObjectProperty})liant des individus à d'autres
  individus, et les propriétés de types de données liant des individus
  à des valeurs de données (\texttt{owl:DatatypeProperty}). Toutes
  deux sont des sous-classes de la classe \acrshort{rdf}
  \texttt{rdf:Property}.
...
\begin{lstlisting}[language={XML}, numbers=none, backgroundcolor=\color{white},]
<author rdf:resource="http://www.w3.org/People/Berners-Lee">
\end{lstlisting}
\end{description}
\enddescription
