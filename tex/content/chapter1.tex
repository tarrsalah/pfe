...
La découverte des services Web est un sujet abordée dans un grand
nombre de projets et travaux de recherche \cite{elie2010}, plusieurs
...
décrivent le processus de découverte comme étant \textit{``la
  ...
  service éventuellement inconnu au préalable écrivant certains
  critères fonctionnels ''} \cite{booth2004web}.

Á l'instar de booth \cite{booth2004web}, Ankolekar \textit{el el.} ont
défini la découverte comme étant l'acte de \textit{``localiser les
  services Web (généralement par le biais d'un annuaire des services)
  fournissant un service particulier et qui adhèrent aux contraintes
  spécifiées''} \cite{ankolekar2002daml}.
...
\end{itemize}
  ...
  approches centralisées et les approches distribuées
  ...
  autre part \cite{elie2010,garofalakis2004web}.
    ...
    % TODO: mentionner UDDI comme une approche purement manuelle
    Dans cette catégorie nous pouvons citer L'approche de découverte
    proposée par Verma \textit{el al.} \cite{verma2005meteor} dans le
    cadre du project \textsc{METEOR-S}, l'infrastructure présente un
    degré d'automatisation plus avancé qu'une simple recherche dans un
    ...
    % TODO: manual vs semi-auto découverte
    ...
    dynamique et évolutive des services Web.
    ...
    modèle \textsc{WSMO} (voir \ref{sec:wsmo}).

    % Le modèle proposé traite à la fois l'aspect statique et l'aspect
    % dynamique de la description des services Web. L'aspect statique
    % consiste à trouver les correspondances entre les descriptions des
    % services et une requête utilisateur. L'aspect dynamique raffine le
    % choix e des services pré-sélectionnés durant la phase statique en
    % interrogeant le fournisseur pour vérifier certaines conditions
    % d'utilisation telles que les pré-conditions, les effets et la
    % disponibilité.
    \newpage
  \subsection{Matching des services Web}
  \label{sec:ws-matching} Le processus du matching (ou le matchmaking)
  repose sur la recherche de similarités entre les paramètres
  descriptives de ces derniers. Il existe deux catégories de
  paramètres de description des services web: les paramètres
  fonctionnels et les paramètres non-fonctionnels que nous décrivons
  tout de suite.
