...
{\color{red}
  \begin{itemize}
  \item Défnir le processus de matching, alignement et mapping.
  \item clarifier la liaison entre le matching sémantique et la
    découverte automatique des services Web.
  \item Le matching syntaxique vs matching sémanique
  \item matching syntaxique
  \item matching sémanique
  \end{itemize}
}

% Dans le contexte de la découverte des services Web. Le processus du
% matching (ou le \textit{matchmaking}) repose sur la recherche de
% similarités entre les paramètres descriptives fonctionnels et les
% paramètres non-fonctionnels des services. \ref{sec:ws-description}.

Dans le contexte d'une architecture à services, le \textit{Matching}
...
spécifiées dans la requête d'entrée
\cite{li2004software}. Formellement, Le processus de
\textit{Matchmacking} peut se spécifié comme de suit:

\begin{mydef}[Matching]
  c'est le processus de découverte des liaisons et des correspondances
  entre les entités de différentes représentations à travers un
