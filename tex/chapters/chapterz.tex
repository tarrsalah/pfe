...
\newpage
  ...
  stockage en termes des nœuds et des arcs directement.
    ...
    % (ce qui est démontré dans la figure \ref{fig:periscope-gq}).
    ...
    propriété fondamentale appelée \emph{index-free adjacency}. Dans
    cette sous-section on va présenter quelques techniques de stockage
    interne et d'indexation utilisées par les système de gestion des
    ...
    \item [Vertex Centric Indices] (\emph{Titan} \cite{vertexci}):
      ...
      propriétés de ces arcs. Ces indexes élimine la nécessité de
      balayage linéaire des arcs incidents lors d'un requête de
      parcours ($\mathcal{O}(n)$) en faveur d'un parcour plus rapide (
      ...
      noeud avec un trés grans nombre des arcs incidents.
    ...
    \item [Bitmaps representation of graphs]: Graph database like DEX
      ...
      add an edge between two nodes, it just needs to update the
      related bit to 1. DEX also uses key-value maps to provide the
      index of full data access to complement the bitmaps. Because the
      adjacency is usually sparse, the compressed bitmaps will use the
      space more efficiently. Another advantage is that the insert,
      delete operations are all bit operations, which are much faster
      than common operation for objects.
    \end{itemize}
