...
Une service Web composite $\mathpzc{S = <I_S, O_S>}$ est créer et
exécuter suite à une requête de composition $\mathpzc{Q=<I_Q, O_Q>}$.
Deux conditions nécessaires et suffisantes pour que $\mathpzc{S}$
...
La relation ``$\subseteq$'' désigne la relation de
\emph{``subsomption`''} entre les interfaces d'\emph{entrée/sortie},
cette notion sera abordée dans la section~\ref{sec:ch4/matching}.

\begin{mydef}[\textbf{Problème de composition}]
\end{mydef}


\section{L'architecture proposée}
\label{sec:proposition}

\section{Le matching des services Web atomiques}
\label{sec:ch4/matching}

\section{La construction et la persistance d'un graphe de dépendance
  de composition}

\section{La découverte, la publication et l'exécution d'un service Web
  composite}
