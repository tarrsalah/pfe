...
\end{itemize}
  ...
  simultanément aux trois contraintes suivantes: \textit{(i)}
  \textsf{Cohérence}: tous les nœuds du système voient exactement les
  mêmes données au même moment. \textit{(ii)} \textsf{Disponibilité}:
  la perte de nœuds n'empêche pas les survivants de continuer à
  fonctionner correctement. \textit{(iii)} \textsf{Résistance au
    partitionnement}: aucune panne moins importante qu'une coupure
  totale du réseau ne doit empêcher le système de répondre
  correctement.\medskip
...
\label{sec:graph-database-overview}
  ...
  % nœuds reliés par des arcs ou des arêtes). Depuis plusieurs
  % décennies, les développeurs ont essayé de stocker des ensembles de
  % données connectés, \textit{semi-structurées} à l'intérieur des bases
  % de données relationnelles. Mais alors que les bases de données
  ...
  % réseau résulte en une opération de jointure dans le
  % \acrshort{SGBDR}. Implémentées comme des opérations ensemblistes
  % entre l'ensemble des clés primaires de deux tables, les jointures
  % sont des opérations lentes et sans capacité à monter en charge alors
  % que le nombre de tuples de ces tables augmente
  % \cite{robinson2013graph}.\medskip

  % des pointeurs physiques ce qui n'est pas le cas des clefs étrangères
  % (bases relationnelles) qui offrent uniquement des pointeurs
  % logiques.
...
données graphe est grandissant. Ces systèmes, dans la mouvance des
systèmes \acrshort{nosql}, permettent de manipuler des données
structurées sous forme de graphe, auxquelles aucun schéma n'est a
priori associée. Ainsi, les systèmes de gestion de bases de données
graphe permettent entre autres la gestion de données de réseaux
sociaux, d'images, de vidéos, de données \textit{bio-informatiques},
et de données issues du Web sémantique telles que des données au
format \acrshort{rdf} éventuellement issues de l'exploitation de
données ouvertes. De nombreux systèmes permettant la gestion de bases
de données graphe sont apparus.  Il s'agit par exemple de
\textit{AllegroGraph} \textit{InfiniteGraph}, \textit{Neo4j} ou
\textit{Sparksee}.\medskip
...
\ref{sec:graph-internals}.\medskip
  ...
  \label{sec:graphdb-defs}
   ...
   Angles et Gutierrez \cite{angles2008survey} conceptualise
  la notion d'une base de données graphe (\textit{``graph db-model''})
  par rapport à trois éléments fondamentaux:\medskip
  ...
  \item [Modèle de données]: Une base de données graphe stocke les
    ...
    des \textit{relations}.\medskip

    Les nœuds représentent généralement les entités

  \item [Manipulation de données]: Les manipulation de données est
    exprimée par des transformations de graphe
