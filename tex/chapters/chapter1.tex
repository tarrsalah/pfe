...
Plusieurs paradigmes de logiciels ont été proposés
...
\ref{sec:ws-standards}. Ensuite nous focalisons sur les modèles de
...
la littérature.
  ...
  focalisons sur les services Web (ou plus clairement
  ...
  le cas pour les scripts \acrshort{cgi} ou les applications
  ...
  \item[Interopérables]: Le standard \acrshort{soap}
    ...
    \acrshort{http} et sérialisé en \acrshort{w3c}, permettant le
    support pour l'interopérabilité.

  \item[Basés sur XML] : Le métalangage de balisage \acrshort{w3c}
  ...
  \item [L'interopérabilité:] l'interopérabilité permet à des
    ...
    et s'exécutant sur des plates-formes différentes de communiquer
    entre elles.
...
\newpage
  ...
  d'échange des données indépendant du modèle de programmation de
  ...
  \item[<Interface>]: Les interfaces \acrshort{wsdl} offrent une
    ...
    paramètres d'entrée et de sortie sous forme d'un un modèle
  ...
  en permettant de créer un \textit{annuaire} permettant de localiser
  sur le réseau les services web recherchés, les services référencés
  ...
  d'entreprises. Les données stockés dans l'\acrshort{uddi} sont
...
abordée brièvement dans l'annexe \ref{annexe:semantic-web}.
  ...
  \cite{elie2010}.
    ...
    \item [modelreference:] Un attribut qui peut être ajouté à élément
      dans une grammaire \acrshort{xml} et aux élément
  ...
  dans cette sous-section \textsc{OWL-S} et \acrshort{wsmo}.
    ...
    Pour formaliser \acrshort{wsmo}, le groupe de travail mis au point
    ...
    sous-ontologies: un \emph{service profile}, d'un \emph{service
...
approches décentralisées ou distribuées \cite{garofalakis2004web}.
    ...
    de schéma globale de la fédération. Les nœuds d'un annuaire
    collaborent pour gérer un ensemble de structures de données
    \acrshort{uddi} et permettre une meilleure gestion des requêtes.

  \subsection{Découverte manuelle/automatique}
  \label{sec:ws-desc:manual-vs-auto}
  Selon le richesse sémantique de la description de services, le degré
  d'intervention de l'utilisateur dans la découverte varié. Nous
  pouvons distinguer les approches manuelles et semi-automatique d'une
...
(\acrshort{soa}). Ils sont des composants logiciels
