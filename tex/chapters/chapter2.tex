...
la littérature.
  ...
  \textit{SGD} avec quelques modifications.\newpage
  ...
  La principale contribution est la manière de traiter les cas de
  boucle dans un graphe, ce problème particulièrement a été très peu
  abordé dans ce domaine.\bigskip
  ...
  \cite{arpinar2005ontology} se base sur la programmation dynamique et
  l'algorithme de \textit{Bellman-Ford} pour la recherche du plus
  court chemin en optimisant le temps d'exécution et le degré de
  similarité sémantique entre les services.\bigskip

  % (parmi les premiers travaux abordant la composition de ce point de
  % vue) utilise l’algorithme de Bellman de la programmation dynamique
  % pour la recherche du plus court chemin en optimisant le temps
  % d’exécution et le degré de similarité entre les services.
  ...
  \textsc{OWLS} pour modélise le graphe de dépendance, ce dernier est
  construit avant toute requête soumise par ...
  ...
  sur l'algorithme de Floyd-Warshall, en optimisant le degré du
  matching sémantique, temps d'exécution, coût et la disponibilité du
  service.
